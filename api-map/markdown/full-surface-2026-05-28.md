...
> **Historical capture—not current capability truth.** Preserve this for route
> provenance. Current auth and write behavior lives in
...
At capture time, writes were documented as dry-run-only and used an older
`--live-write` convention. Do not copy that convention into current commands.
