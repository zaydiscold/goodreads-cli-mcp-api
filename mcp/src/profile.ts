...
export type McpProfile = "full" | "core" | "notes" | "read";
...
const PROFILE_TOOLS: Record<McpProfile, ReadonlySet<GoodreadsToolName>> = {
  ...
  read: new Set(READ_TOOL_NAMES),
};

export function parseMcpProfile(value: string | undefined): McpProfile {
  const profile = value?.trim() || "read";
  if (profile === "full" || profile === "core" || profile === "notes" || profile === "read") {
    return profile;
  }
  throw new Error(
    `GOODREADS_MCP_PROFILE must be one of full, core, notes, read (received ${JSON.stringify(profile)})`,
  );
}
