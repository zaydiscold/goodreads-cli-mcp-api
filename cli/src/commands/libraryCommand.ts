...
export function libraryCommand(): Command {
  ...
  cmd
    .command("set-status")
    .description("Set reading status via POST /shelf/add_to_shelf.")
    .requiredOption("--book-id <id>")
    ...
    .action(async (o) =>
      out(
        await ru({
          ...
          rating: star(o.rating),
          approvedBookId: o.approvedBookId,
          approvedRating: star(o.approvedRating),
          execute: Boolean(o.execute),
        }),
      ),
    );
  cmd
    .command("review")
    .description("Create/update review via POST /review/update/{book_id}.")
    .requiredOption("--book-id <id>")
    .option("--text <text>")
    .option("--approved-book-id <ids...>")
